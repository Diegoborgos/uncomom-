 ...
 * Only shows when city.signals is populated. Gracefully returns null otherwise.
 */
...
function PassportRow({ tier, examples, days, approval, friendly }: {
  ...
  const color = friendly === "excellent" ? "var(--accent-green)"
    : friendly === "good" ? "var(--score-mid)"
    : friendly === "ok" ? "var(--accent-warm)"
    : "var(--score-low)"

  return (
    <div className="rounded-lg bg-[var(--surface-elevated)] px-4 py-3 flex items-center justify-between">
      <div>
        <p className="text-sm font-medium">{tier} passport</p>
        <p className="text-[10px] text-[var(--text-secondary)]">{examples}</p>
      </div>
      <div className="flex items-center gap-4 text-right">
        <div>
          <p className="text-sm font-mono">{days}d</p>
          <p className="text-[9px] text-[var(--text-secondary)]">processing</p>
        </div>
        <div>
          <p className="text-sm font-mono">{approval}%</p>
          <p className="text-[9px] text-[var(--text-secondary)]">approval</p>
        </div>
        <span className="text-[10px] px-2 py-0.5 rounded-full font-medium capitalize" style={{ backgroundColor: color + "22", color }}>
          {friendly}
        </span>
      </div>
    </div>
