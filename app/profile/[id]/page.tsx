    ...
    const load = async () => {
      try {
        ...
        try {
          ...
          // family_follows table may not exist yet
        }
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-16 relative z-10">
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {mostVisits.slice(0, 8).map(([slug, count]) => {
                ...
                if (!city) return null
                return <ProfileCityCard key={slug} city={city} badge={`${count}x`} href={`/cities/${slug}`} />
              })}
            </div>
          </section>
        )}
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {mostTime.slice(0, 8).map(([slug, days]) => {
                const city = cities.find(c => c.slug === slug)
                if (!city) return null
                return <ProfileCityCard key={slug} city={city} badge={`${days}d`} href={`/cities/${slug}`} />
              })}
            </div>
          </section>
        )}
        ...
        {crossPaths.length > 0 && (
          <section className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {crossPaths.slice(0, 20).map(f => <AvatarLink key={f.id} family={f} />)}
            </div>
          </section>
        )}
...
function ProfileCityCard({ city, badge, href }: { city: { name: string; country: string; countryCode: string; photo: string; scores: { family: number }; cost: { familyMonthly: number } }; badge: string; href: string }) {
  return (
...
function AvatarLink({ family }: { family: CrossPathFamily }) {
  const href = `/profile/${family.username || family.id}`
  const initials = family.family_name?.slice(0, 2).toUpperCase() || "??"
  return (
